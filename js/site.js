...
(function () {
  if (window.FI) return; /* 한 페이지에서 여러 컴포넌트가 불러도 단 한 번만 초기화 — 리스너 공유 */
  var KEY = 'fi_lang';
  ...
  var PROJECTS = [
    {
      ...
      desc: {
        ...
        en: 'A lightweight investment tool — search a ticker, check live-style prices, FX and indices, then simulate buy → dividend → reinvest month by month and compare reinvest vs. no-reinvest side by side. No account required, works the same on web and mobile.'
      },
      ...
      features: [
        ...
        { k: '반응형', t: { ko: '웹·모바일 반응형', en: 'Responsive web & mobile' }, d: { ko: '데스크톱은 2열, 모바일은 카드형으로 정보 밀도를 자동 전환합니다.', en: 'Two-column on desktop, card layout on mobile — density adapts to the viewport.' } }
      ],
      stats: [
    ...
    },
    {
      ...
      desc: {
        ...
        en: 'Record your unit with a checklist and photos at move-in and move-out, and the app produces a PDF evidence pack — auto-stamped photos plus a before/after comparison. Offline-first and stored safely on-device with no server, so you are ready if a deposit dispute arises.'
      },
      ...
      features: [
        ...
        { k: '오프라인', t: { ko: '오프라인 우선 저장', en: 'Offline-first storage' }, d: { ko: '서버 없이 기기(sqflite)에 저장되어 인터넷 없이도 기록하고 보관합니다.', en: 'Stored on-device (sqflite) with no server — record and keep everything without a connection.' } }
      ],
      stats: [
